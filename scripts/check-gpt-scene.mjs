...
for (const tokens of tokenSets) {
  ...
  for (let selectedLayer = 0; selectedLayer < 4; selectedLayer += 1) {
    for (const expanded of [false, true]) {
      const selectedOperations = expanded
        ? STACK_OPERATION_STEPS.map((operation) => operation.id)
        : [STACK_OPERATION_STEPS[0].id];
      for (const selectedOperation of selectedOperations) {
        for (const selectedToken of [...new Set([0, tokens.length - 1])]) {
          ...
          if (errors.length) {
            throw new Error(
              [
                `Invalid stack scene for ${tokens.length} tokens, block ${
                  selectedLayer + 1
                }, ${selectedOperation}, ${expanded ? 'open' : 'collapsed'}:`,
